...
Корневой `CHANGELOG.md` содержит semantic version, дату `DD.MM.YYYY`, рекомендуемое время с UTC offset и короткое название релиза; встроенная история приложения строится из него. Канонический заголовок: `## [3.2.0] — 29.08.2026 18:40 +02:00 — Rewards Lab и грейды задач`. Совпавший GitHub Release служит более точным автоматическим источником `published_at`. Для унаследованного пути допускается задокументированный adapter (например, MonoFocus), но новые проекты начинают с корневого файла. Записи только с датой и старые ISO-даты читаются для совместимости.
...
- `theme_color` в manifest и `<meta name="theme-color">` используют один непрозрачный тёмный цвет системной панели и верхней поверхности мобильного приложения;
- приложение не задаёт светлый `color-scheme`, пока физическое устройство не подтвердило светлый фон системной панели: Pixel 11 Pro сохранил тёмный фон, но сделал индикаторы чёрными;
- мобильный интерфейс не дублирует обязательную системную панель отдельной шапкой, если в ней нет уникального критического действия; глобальное меню доступно из нижней навигации;
...
- практика станет обязательной только после повторной проверки тёмного пилота Верфи на Pixel 11 Pro.
